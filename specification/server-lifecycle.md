...
## Purpose
Orchestrate the correct startup order of the HTTP server and SSDP components. The upstream `async_upnp_client` library creates the device (and sends SSDP advertisements) before starting the HTTP server, which causes SSDP LOCATION URLs to contain port 0 when auto-assigning ports.
...
5. Return ServerHandle(port, responder, announcer, runner)
```
...
#### `async stop()`
Shutdown sequence:
1. Stop advertisement announcer (stop sending NOTIFY)
2. Stop search responder (stop responding to M-SEARCH)
3. Cleanup aiohttp AppRunner
...
4. Call `start_server()` with all parameters
5. Wait for SIGINT/SIGTERM
6. Call `ServerHandle.stop()` to cleanly shut down
